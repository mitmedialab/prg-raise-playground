 ...
 */
  ...
  private initEventListeners(): void {
    ...
    if (flyout) {
      ...
      setInterval(() => {
        ...
        if (cursor) {
          ...
          if (curNode) {
            const block = curNode as Blockly.BlockSvg;
            if (block) {
    ...
    // Flyout focus
    const flyoutElement = this.workspace.getFlyout()?.getWorkspace().getParentSvg();
    if (flyoutElement) {
 ...
 */
  ...
  private getNodeIdentifier(node: Blockly.IFocusableNode | null): string {
    if (node instanceof Blockly.Block && !node.getNextBlock()) {
      ...
      const field = node as Blockly.Field;
      const block = field.getSourceBlock();
      return `field-${block?.id || 'unknown'}-${field.name}`;
    } else if (node instanceof Blockly.Input) {
      const input = node as Blockly.Input;
  ...
  */
  ...
  private announceNode(node: Blockly.IFocusableNode | null): void {
    ...
    if (node instanceof Blockly.Block && !node.getNextBlock()) {
      ...
      this.speakHighPriority(`Unknown element: ${node}`);
    }
  }
