...
const TextBoxStyle = {
    ...
    STROKE_WIDTH: 0, // Thickness of the stroke around the bubble. Only half's visible because it's drawn under the fill
    PADDING: 10, // Padding around the text area
    CORNER_RADIUS: 0, // Radius of the rounded corners
    TAIL_HEIGHT: 0, // Height of the speech bubble's "tail". Probably should be a constant.
...
class TextBoxSkin extends Skin {
    /**
     ...
     */
    setTextBox (type, text, pointsLeft) {
        this._text = text;
        this._textBoxType = type;
     ...
     */
    _restyleCanvas () {
        this._canvas.getContext('2d').font = `${TextBoxStyle.FONT_SIZE}px ${TextBoxStyle.FONT}, sans-serif`;
    }
     ...
     */
    _reflowLines () {
        this._lines = this.textWrapper.wrapText(TextBoxStyle.MAX_LINE_WIDTH, this._text);
        ...
        const paddedWidth = Math.max(longestLineWidth, TextBoxStyle.MIN_WIDTH) + (TextBoxStyle.PADDING * 2);
        const paddedHeight = (TextBoxStyle.LINE_HEIGHT * this._lines.length) + (TextBoxStyle.PADDING * 2);

        this._textAreaSize.width = paddedWidth;
        this._textAreaSize.height = paddedHeight;

        this._size[0] = paddedWidth + TextBoxStyle.STROKE_WIDTH;
        this._size[1] = paddedHeight + TextBoxStyle.STROKE_WIDTH + TextBoxStyle.TAIL_HEIGHT;
    ...
    _renderTextBox (scale) {
        ...
        ctx.translate(TextBoxStyle.STROKE_WIDTH * 0.5, TextBoxStyle.STROKE_WIDTH * 0.5);
        ...
        ctx.beginPath();
        ctx.moveTo(TextBoxStyle.CORNER_RADIUS, paddedHeight);
        ctx.arcTo(0, paddedHeight, 0, paddedHeight - TextBoxStyle.CORNER_RADIUS, TextBoxStyle.CORNER_RADIUS);
        ctx.arcTo(0, 0, paddedWidth, 0, TextBoxStyle.CORNER_RADIUS);
        ctx.arcTo(paddedWidth, 0, paddedWidth, paddedHeight, TextBoxStyle.CORNER_RADIUS);
        ctx.arcTo(paddedWidth, paddedHeight, paddedWidth - TextBoxStyle.CORNER_RADIUS, paddedHeight,
            TextBoxStyle.CORNER_RADIUS);

        // Translate the canvas so we don't have to do a bunch of width/height arithmetic
        ctx.save();
        ctx.translate(paddedWidth - TextBoxStyle.CORNER_RADIUS, paddedHeight);

        // Draw the bubble's "tail"
        if (this._textBoxType === 'say') {
        ...
        ctx.fillStyle = TextBoxStyle.COLORS.BUBBLE_FILL;
        ctx.strokeStyle = TextBoxStyle.COLORS.BUBBLE_STROKE;
        ctx.lineWidth = TextBoxStyle.STROKE_WIDTH;
        ...
        ctx.fillStyle = TextBoxStyle.COLORS.TEXT_FILL;
        ctx.font = `${TextBoxStyle.FONT_SIZE}px ${TextBoxStyle.FONT}, sans-serif`;
        ...
        for (let lineNumber = 0; lineNumber < lines.length; lineNumber++) {
            ...
            ctx.fillText(
                ...
                TextBoxStyle.PADDING + (TextBoxStyle.LINE_HEIGHT * lineNumber) +
                    (TextBoxStyle.FONT_HEIGHT_RATIO * TextBoxStyle.FONT_SIZE)
            );
        }
    ...
    getTexture (scale) {
        ...
        if (this._textureDirty || this._renderedScale !== requestedScale) {
            this._renderTextBox(requestedScale);
