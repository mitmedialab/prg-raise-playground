...
class Scratch3LooksBlocks {
    ...
    _positionBubble(target) {
        ...
        if (
            ...
            this.runtime.renderer.updateDrawableProperties(
                bubbleState.drawableId,
                {
                    position: [
                        bubbleState.onSpriteRight
    ...
    think(args, util) {
        this._updateBubble(
            util.target,
            "think",
            String(args.MESSAGE).substr(0, Scratch3LooksBlocks.SAY_BUBBLE_LIMIT)
        );
    }
            ...
            if (costumeIndex !== -1) {
                ...
                if (numCostumes > 1) {
                    let selectedIndex = Math.floor(
                        Math.random() * (numCostumes - 1)
                    );
                    if (selectedIndex === stage.currentCostume)
                        selectedIndex += 1;
