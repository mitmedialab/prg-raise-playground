 ...
 */
type Blocks = {
  exampleCommand(argument: string): void; // Can delete, just an example
 ...
 * 
 * As mentioned above, this `Extension` class takes 2 generic type arguments, which tell us (and Typescript, and the Extension Framework) what this extension is all about.
 * The first generic argument tells us how this extension should be display in the Extensions Menu;
 ...
 * Typescript holds us accountable to implement exactly what we said we would (in order to make a working extension).
 *  
 * This includes:
 * - Defining an `init` function (see below) that is used INSTEAD of a constructor
 * -  
 */
class ExtensionNameGoesHere extends Extension<Details, Blocks> {
  ...
  defineBlocks(): ExtensionNameGoesHere["BlockDefinitions"] {
    return {
      exampleCommand: (self: ExtensionNameGoesHere) => ({
        // Hover over each of the below fields ('type', 'args', 'text', 'operation') 
        // to see what that field means and what values it can take on
      ...
      exampleReporter: () => ({
        ...
        // NOTE: There's no args field since this is not defining a 
        type: BlockType.Reporter,
        text: "Replace with block's display text",
        operation: () => {
          // Replace with what the block should do! 
          return 789;
        }
