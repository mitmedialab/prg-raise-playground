...
type Blocks = {
  ...
  testBlock() : string;
  functionBlock(name: string) : string;
...
class ExtensionNameGoesHere extends Extension<Details, Blocks> {
  ...
  defineBlocks(): ExtensionNameGoesHere["BlockDefinitions"] {
    ...
    const testBlock: DefineExampleCommand = () => ({
      type: BlockType.Command,
      args: [ArgumentType.String, { type: ArgumentType.Number, defaultValue: 789 }],
      text: () => `Hello test block`,
      operation: () => {
        alert(this.returnAWord()); // Replace with what the block should do! 
      }
    });
    ...
    return {
      ...
      functionBlock : function (self: ExtensionNameGoesHere) : Block<Blocks["functionBlock"]> {
        return {
          type: BlockType.Reporter,
          arg: ArgumentType.String,
          text: (name : string) => `Type in your name ${name}`,
