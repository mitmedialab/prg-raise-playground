...
class Scratch3TextClassificationBlocks {
    constructor(runtime) {
        ...
        this.scratch_vm.on("CLEAR_ALL_LABELS", () => {
            if (
                !this.labelListEmpty &&
                confirm("Are you sure you want to clear all labels?")
            ) {
     ...
     */
    _getState(target) {
        let state = target.getCustomState(
            Scratch3TextClassificationBlocks.STATE_KEY
        );
        if (!state) {
            state = Clone.simple(
                Scratch3TextClassificationBlocks.DEFAULT_TEXT2SPEECH_STATE
            );
    ...
    getInfo() {
        ...
        return {
            ...
            name: formatMessage({
                ...
                description:
                    "Label for the Text Classification extension category",
            }),
            ...
            blocks: [
                ...
                },
                {
                    ...
                    text: formatMessage({
                        ...
                        description:
                            "Conditional that is true when the text matches the text classification model class [CLASS_NAME]",
                    }),
                ...
                },
                {
                    ...
                    text: formatMessage({
                        ...
                        description:
                            "Get the class name that the input text matches",
                    }),
                ...
                },
                {
                    ...
                    text: formatMessage({
                        ...
                        default: "Confidence of predict class for [TEXT]",
                        description:
                            "Get the Confidence level of class name that the input text matches",
                    }),
                    blockType: BlockType.REPORTER,
                    arguments: {
                        TEXT: {
                ...
                "---",
                {
                    ...
                    text: formatMessage({
                        ...
                        description:
                            "get the similarity between two words/sentences",
                    }),
                    blockType: BlockType.REPORTER,
                    arguments: {
                        TEXT_ONE: {
                ...
                },
                {
                    ...
                    text: formatMessage({
                        ...
                        default: "response",
                        description:
                            "Return the results of the speech recognition",
                    }),
                    blockType: BlockType.REPORTER,
                },
                {
                ...
                },
                {
                    ...
                    text: formatMessage({
                        ...
                        description:
                            "Event that triggers when a sound is heard above a threshold",
                    }),
     ...
     */
    newExamples(text_examples, label) {
        ...
        for (let text_example of text_examples) {
            if (
                !this.scratch_vm.modelData.textData[label].includes(
                    text_example
                )
            ) {
                this.scratch_vm.modelData.textData[label].push(text_example);
                this.scratch_vm.modelData.classifierData[label].push(
                    text_example
                );
            }
        }
    ...
    renameLabel(oldName, newName) {
        ...
        this.scratch_vm.modelData.classifierData[newName] =
            this.scratch_vm.modelData.classifierData[oldName]; //reset the runtime's model data with the new renamed label (to share with GUI)
        delete this.scratch_vm.modelData.classifierData[oldName];

        this.scratch_vm.modelData.textData[newName] =
            this.scratch_vm.modelData.textData[oldName]; //reset the runtime's model data with the new renamed label (to share with GUI)
    ...
    deleteExample(label, exampleNum) {
        ...
        if (exampleNum === -1) {
            ...
            let numLoadedExamples =
                this.scratch_vm.modelData.classifierData[label].length -
                this.scratch_vm.modelData.textData[label].length; //imageData[label].length is ONLY the length of the NEW examples (not the saved and then loaded ones!)
            this.scratch_vm.modelData.classifierData[label].splice(
                0,
                numLoadedExamples
            );
        } else {
            ...
            this.scratch_vm.modelData.classifierData[label].splice(
                exampleNum -
                    this.scratch_vm.modelData.textData[label].length -
                    1,
                1
            );
        }
    ...
    async getSimilarity(args) {
        if (this._useModel) {
            ...
            const distance = tf.losses
                .cosineDistance(firstEmbedding, secondEmbedding, 1)
                .dataSync();
    ...
    async ifTextMatchesClass(args, util) {
        ...
        const predictionState = await this.get_embeddings(
            text,
            "none",
            "predict"
        );
    ...
    async getModelConfidence(args) {
        const text = args.TEXT;
        const predictionConfidence = await this.get_confidence(
            text,
            "none",
            "predict"
        );
    ...
    getPredictedClass(text, className) {
        if (!this.labelListEmpty) {
            ...
            try {
                for (let example of this.scratch_vm.modelData.textData[
                    className
                ]) {
            ...
            //if there is no data in the classifier
            return false;
        }
    }
     ...
     * @returns if the direction is "predict" returns the predicted label for the text inputted
     */
    async get_confidence(text, label, direction) {
        //changes text into a 2d tensor
        const newText = await this.getTranslate(text, "en"); //translates text from any language to english
        console.log(newText);
    ...
    async buildCustomDeepModel() {
        ...
        this.scratch_vm.emit(
            "SAY",
            this.scratch_vm.executableTargets[1],
            "think",
            "wait .. loading model"
        );
        ...
        const ys = tf.oneHot(
            tf.tensor1d(
                this.labledsample.map((a) =>
                    this.labelList.findIndex((e) => e === a)
                ),
                "int32"
            ),
            numClass
        ...
        const trainingData = await use
            .load()
            .then((model) => {
                return model.embed(this.sentencesample).then((embeddings) => {
                    return embeddings;
                });
            })
    ...
    async predictScore(text) {
        ...
        console.log(text);
        var test_ex = [text];
        const testData = await use
            .load()
            .then((model) => {
                return model.embed(test_ex).then((embeddings) => {
                    return embeddings;
                });
            })
            .catch((err) => console.error("Fit Error:", err));
     ...
     */
    exportClassifier() {
        //exports classifier as JSON file
    ...
    getTranslate(words, language) {
        ...
        if (
            this._lastTextTranslated === words &&
    ...
    _loadToxicityModel() {
        if (this._toxicitymodel) {
            ...
            toxicity
                .load(
                    threshold,
                    this._toxicity_labels.items.map((lbl) => lbl.value)
                )
                .then((model) => {
            ...
            return this._toxicitymodel
                ...
                .then((predictions) => {
                    ...
                    if (
                        ...
                        filtered.length === 1 &&
                        filtered[0].results &&
                        filtered[0].results.length > 0
                    ) {
                        const idx = returnPositive ? 1 : 0;
                        return Math.round(
                            filtered[0].results[0].probabilities[idx] * 100
                        );
                    }
                    return 0;
                })
