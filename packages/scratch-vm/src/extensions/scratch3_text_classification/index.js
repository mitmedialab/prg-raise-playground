...
// delete this
const HF_API_TOKEN = "INSERT API KEY HERE";
...
class Scratch3TextClassificationBlocks {
    constructor (runtime) {
        ...
        // When a project is loaded, reset all the model data, load conversation model
        this.scratch_vm.on('PROJECT_LOADED', () => {
            this.clearLocal();
            this.loadModelFromRuntime();
            fetch(`https://api-inference.huggingface.co/models/${this.model}`, {headers: {"Authorization": `Bearer ${HF_API_TOKEN}`}, method: "POST", body: JSON.stringify('Loading model test')});
        });
        ...
        return {
            ...
            blocks: [
                ...
                },
                {
                    ...
                    text: formatMessage({
                        id: 'textClassification.startConversation',
                        default: '[TEXT] to start conversation',
                ...
                {
                    opcode: 'getConversationAnswer',
                    text: formatMessage({
                        id: 'textClassification.getConversationAnswer',
                        default: 'answer',
                        description: 'Return the reply from the conversation bot'
                    }),
                    blockType: BlockType.REPORTER
                },
                {
    ...
    */ 
    ...
    /**
     * Block that allows a user to communicate with a conversation bot
     * @param {object} args - the block arguments
     * @returns {string} the reply generated by the conversation API
     */
    async startConversation(args) {
        const self = this;
        const data = args.TEXT;
        const newData = await this.getData(data);
        // console.log(newData.generated_text);
        self.replytoask = newData.generated_text;
    }

    /**
     * Helper function to send API request to conversation bot
     * @param {object} data - the text inputted by the user into the conversation model
     * @returns {string} the response generated by the conversation model
     */
    async getData(data) {
        // const model = 'af1tang/personaGPT';
        const response = await fetch(`https://api-inference.huggingface.co/models/${this.model}`, {headers: {"Authorization": `Bearer ${HF_API_TOKEN}`}, method: "POST", body: JSON.stringify(data)});
        return await response.json();
    }

    /**
     * Block to return answer generated by the conversation model
     * @returns {string} the response generated by the conversation model
     */
    getConversationAnswer () {
        return this.replytoask;
    }
