...
class Scratch3TextClassificationBlocks {
            ...
            blocks: [
                ...
                },
                {
                    ...
                    text: formatMessage({
                        ...
                        description:
                            "get the confidence for the labeling of a specified text",
                    }),
                    blockType: BlockType.REPORTER,
                    arguments: {
                        TEXT: {
    ...
    newExamples(text_examples, label) {
        ...
        for (let text_example of text_examples) {
            if (
                ...
                const embeddedexample = this.getembeddedwords(
                    text_example,
                    label,
                    "example"
                );
    ...
    async getembeddedwords(text, label, direction) {
        if (!this.labelListEmpty) {
            const embeddedtext = await this.get_embeddings(
                text,
                label,
                direction
            );
    ...
    async get_embeddings(text, label, direction) {
        ...
        if (!this.labelListEmpty) {
            ...
            if (direction === "example") {
                ...
                // this formulation for k seems decent
                let k = Math.sqrt(this.count);

                let prediction = await this.classifier.predictClass(
                    textEmbeddings,
                    k
                );

                console.log("get knn result:", prediction);                
                // tie breaker, if there's a tie choose k=1 the nearest neighbor
                for (const label in prediction.confidences) {
                    const conf = prediction.confidences[label];
                    if (label !== prediction.label && conf == prediction.confidences[prediction.label]) {
                        console.log("knn had a tie");
                        prediction = await this.classifier.predictClass(
                            textEmbeddings,
                            1
                        );
                    }
        ...
        const translatePromise = new Promise((resolve) => {
            nets(
                ...
                (err, res, body) => {
                    ...
                    const translated = JSON.parse(body).result.toLowerCase();
