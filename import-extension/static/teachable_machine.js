...
let webcamHolder = document.getElementsByClassName('stage_stage_1fD7k')[0];
...
console.log(tmImage);
console.log(window);
androidWebcam = new tmImage.Webcam( 480,360, flip);
console.log(androidWebcam);
androidWebcam.setup(constraints)
  .then(() => androidWebcam.play())
  .then(() => {
    Object.assign(androidWebcam.canvas.style, {
      position: 'absolute',
      top: '0',
      left: '0',
      width: '480px',
    ...
    console.log(androidWebcam.canvas);
    webcamHolder.appendChild(androidWebcam.canvas)
    console.log(webcamHolder);
  })
...
async function predict() {
  ...
  window.TeachableMachine.reportResult(JSON.stringify(result));
}
...
function startVideo() {
  if (isVideoMode) {
    navigator.mediaDevices.getUserMedia({
      ...
      .catch(e => {
        window.TeachableMachine.error(ERROR_FAILED_TO_START_VIDEO);
        // console.error(e);
      });
...
function classifyImageData(imageData) {
  if (!isVideoMode) {
    ...
    window.TeachableMachine.error(ERROR_CANNOT_CLASSIFY_IMAGE_IN_VIDEO_MODE);
  }
}
...
function classifyVideoData() {
  if (isVideoMode) {
    ...
    window.TeachableMachine.error(ERROR_CANNOT_CLASSIFY_VIDEO_IN_IMAGE_MODE);
  }
}
...
function setInputMode(inputMode) {
  if (inputMode === "image" && isVideoMode) {
    ...
    window.TeachableMachine.error(ERROR_INVALID_INPUT_MODE);
  }
}
