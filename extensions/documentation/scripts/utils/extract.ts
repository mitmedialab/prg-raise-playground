...
const exportNotFound = (exportName: string, pathToFile: string, exports: readonly ExportSpecifier[], error: (msg: string) => void): Parsed<"snippet"> => {
  const seperator = "\n\t-";
...
const extractFromExport = (exportName: string,
  ...
  if (found.length !== 1) exportNotFound(exportName, pathToFile, exports, error);
