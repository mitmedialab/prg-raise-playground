...
import { Environment, extension, ExtensionMenuDisplayDetails, wrapClamp, fetchWithTimeout, RuntimeEvent, buttonBlock, block, SaveDataHandler, } from "$common";
...
import { Predictor, build, cosineDistance, failure, success } from "./model";
import { PartOfSpeech, getWordsMatchingPartOfSpeech, partsOfSpeech } from "./services/partOfSpeech";
import { getItemOfArray, findMatchesLowercase, splitText } from "./utils";
...
type ArrayEntry = Parameters<typeof getItemOfArray>[0]

export default class TextClassification extends extension(details, "legacySupport", "ui", "indicators", "customSaveData") {
  labels: string[] = [];
  ...
  async init({ runtime }: Environment) {
    const { soundPlayers } = this;
    runtime.on(RuntimeEvent.ProjectStopAll, () => soundPlayers.forEach((soundPlayer) => soundPlayer.stop()));
    runtime.on(RuntimeEvent.TargetWasCreated, tryCopyStateToClone);
  }
  ...
  protected override saveDataHandler = new SaveDataHandler({
    Extension: TextClassification,
    onSave: (instance) => Object.fromEntries(instance.modelData),
    onLoad: (instance, data) => {
      instance.modelData = new Map(Object.entries(data));
      instance.labels = [...instance.modelData.keys()];
    }
  });

  /** Begin Non-legacy Block Methods */
  ...
  @block({
    text: (a, b) => `similarity between ${a} and ${b}`,
    type: "reporter",
    args: [{ type: "string", defaultValue: "text one" }, { type: "string", defaultValue: "text two" }]
  })
  async getSimilarity(a: string, b: string) {
    const distance = await cosineDistance(await getTranslationToEnglish(a), await getTranslationToEnglish(b));
  ...
  @block({
    type: "reporter",
    text: (number, text) => `word ${number} of ${text}`,
    args: [{ type: "string", defaultValue: 1 }, { type: "string", defaultValue: "Here comes the robot" }]
  })
  getWordInString(number: ArrayEntry, text: string) {
    return getItemOfArray(number, splitText(text));
  }

  @block({
    type: "reporter",
    text: (partOfSpeech, number, text) => `${partOfSpeech} ${number} of ${text}`,
    args: [
      { type: "string", defaultValue: "noun", options: partsOfSpeech },
      { type: "string", defaultValue: 1 },
      { type: "string", defaultValue: "My name is Scratch Cat" }
    ]
  })
  getPartOfSpeech(partOfSpeech: PartOfSpeech, number: ArrayEntry, text: string) {
    let words = splitText(text);
    const matches = getWordsMatchingPartOfSpeech(words, partOfSpeech, true);
    return getItemOfArray(number, matches);
  }

  @block({
    type: "reporter",
    text: (number, text, query) => `word ${number} of ${text} like ${query}`,
    args: [
      { type: "string", defaultValue: 1 },
      { type: "string", defaultValue: "No I don't like them" },
      { type: "string", defaultValue: "yes no maybe" }
    ]
  })
  getWordsLike(number: ArrayEntry, text: string, query: string) {
    const matches = findMatchesLowercase(text, query.split(" "));
    return getItemOfArray(number, matches);
  }

  @block({
    text: (text, partOfSpeech) => `${text} contains ${partOfSpeech}`,
    type: "Boolean",
    args: [
      { type: "string", defaultValue: "Spin five times" },
      { type: "string", defaultValue: "number", options: partsOfSpeech }
    ]
  })
  textContainsPartOfSpeech(text: string, partOfSpeech: PartOfSpeech) {
    const words = splitText(text);
    const matches = getWordsMatchingPartOfSpeech(words, partOfSpeech);
    return matches.length > 0;
  }

  @block({
    text: (text, query) => `${text} contains words like ${query}`,
    type: "Boolean",
    args: [
      { type: "string", defaultValue: "Move to the left" },
      { type: "string", defaultValue: "spin move say", }
    ]
  })
  textContainsWordsLike(text: string, query: string) {
    const matches = findMatchesLowercase(text, query.split(" "));
    return matches.length > 0;
  }

  /** End Non-legacy Block Methods */

  /** Begin Legacy Block Methods */
