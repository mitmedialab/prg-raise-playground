...
// Main function to generate and save the WAV file
 createAndSaveWAV(interleaved, sampleRate) {
  ...
  //   }
  // );
  // fetch(`https://www.wavsource.com/snds_2020-10-01_3728627494378403/people/comedians/allen_arrogh.wav`, { mode: 'no-cors' })
  // .then(res => res.blob())
  // .then((myBlob) => {
  ...
  const blob = new Blob([wavData], { type: "audio/wav" });
    ...
    const file1 = new File([blob], "generated.wav", { type: "audio/wav" });
   //console.log(response);
    ...
    // Optionally, create a File object (useful if the server expects a file with a name)
    // const file1 = new File([blob], "audio_file.wav", { type: blob.type });

    const formData = new FormData();
    formData.append("audio_file", file1);
  ...
  @block({
    type: "command",
    text: (seconds) => `Record and play for ${seconds}`,
    arg: { type: "number" }
  })
