...
export function followLine(linePixels: number[][], delay1: number, previousSpeed1: number) {
    ...
    const reference1 = pixelToGroundCoordinates([spline.at(spline.xs[0]), spline.xs[0]])
    const reference2 = pixelToGroundCoordinates([imageDimensions[0]/2, spline.xs[0]])
    const xOffset = reference1.x - reference2.x;
    ...
    console.log("xs");
    console.log(spline.xs);
    const groundCoordinate1 = pixelToGroundCoordinates([spline.at(spline.xs[8]*0.75), spline.xs[8]*0.75]);
    const groundCoordinate2 = pixelToGroundCoordinates([spline.at(spline.xs[8]), spline.xs[8]]);
    ...
    const bezier = new Bezier.Bezier(
        { x: baseCoordinate.x + xOffset, y: baseCoordinate.y },
        { x: baseCoordinate.x + xOffset, y: baseCoordinate.y + bezierIncrement },
        groundCoordinate1,
        groundCoordinate2
    );
    console.log("bezier")
...
function purePursuit(currentPoint: {x: number, y: number, theta: number}, lookaheadPoint: {x: number, y: number, theta: number}) {
    ...
    console.log("ANGLE");
    console.log(angleToLookahead* (180/Math.PI));
