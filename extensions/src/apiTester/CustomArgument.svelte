<script lang="ts">
   ...
   */
  type Value = ParameterOf<Extension, "fireRequest", 0>; 
  ...
  let keys = ["method"];
  let values = ["GET"];
  let url = "https://prg-key-server.netlify.app/.netlify/functions/ai-blocks-drive";

  $: setter({ value: {
    url,
    entries: keys.reduce((acc, key, index) => {
